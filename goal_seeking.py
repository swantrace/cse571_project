...
def get_network_param(sim_env, action, scaler):
    ...
    network_param = np.append(
        sensor_readings, [action, 0]
    )  # unutilized 0 added to match shape of scaler
...
def goal_seeking(goals_to_reach):
    ...
    while goals_reached < goals_to_reach:
        ...
        action, _ = steering_behavior.get_action(
            sim_env.robot.body.position, sim_env.robot.body.angle
        )
