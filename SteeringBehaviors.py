...
class Wander:
    ...
    def get_action(self, timestep_i, current_orientation, actions_checked=[]):
        perlin_noise = noise.pnoise1(
            (float(timestep_i * self.action_repeat) + self.offset0) / self.scale0
        )
        perlin_noise += noise.pnoise1(
            (float(timestep_i * self.action_repeat) + self.offset1) / self.scale1
        )
    ...
    def reset_action(self):
        self.offset0, self.offset1 = random.randint(0, 1000000), random.randint(
            0, 1000000
        )
