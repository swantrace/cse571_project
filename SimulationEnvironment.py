...
class SimulationEnvironment:
    def __init__(self):
        ...
        NORTH, SOUTH, EAST, WEST = (
            ...
            0,
            math.pi,
        )  # PLEASE ONLY USE EAST AT THIS TIME
        ...
        self.wall_shapes = self.assemble_walls(
            self.screen_width, self.screen_height, 180
        )
    ...
    def assemble_walls(self, w, h, u):
        ...
        for wall_points in wall_points_list:
            for i in range(len(wall_points) - 1):
                wall_body, wall_shape = self.build_wall(
                    wall_points[i], wall_points[i + 1], thickness=8
                )
    ...
    def _draw_everything(self, velocity=False, steering=True):
        ...
        img_pos, img_ori = self.pm2pgP(self.robot.body.position), degrees(
            self.robot.body.angle
        )
    ...
    def _apply_robot_motion(self, steering_direction):
        ...
        self.robot.body.apply_force_at_world_point(
            self.steering_force * 10, self.robot.body.position
        )
    ...
    def raycasting(self, print_sensors=False):
        ...
        for i, p in enumerate(sensor_end_points):
            segment_query = self.space.segment_query_first(
                self.robot.body.position, p, 1, robot_filter
            )
            if segment_query:
                segment_queries.append(
                    la.norm(segment_query.point - self.robot.body.position)
                )
            else:
        ...
        if print_sensors:
            print(
                "%d %d %d %d %d"
                % (int(sq[0]), int(sq[1]), int(sq[2]), int(sq[3]), int(sq[4]))
            )
        return sq
    ...
    def _reset_robot(self, center=False, collision_points=None):
        ...
        if collision_points:
            self.robot.body.angle = angle(
                collision_points[0].point_a - collision_points[0].point_b
            )
        else:
    ...
    def turn_robot_around(self):
        previous_angle = vector(self.robot.body.angle)
        previous_position = self.robot.body.position
        steering_vector = -previous_angle + 0.01 * np.random.randn(2)
